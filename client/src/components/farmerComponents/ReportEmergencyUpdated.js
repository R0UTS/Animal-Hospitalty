...
function ReportEmergencyPage() {
    ...
    const onLocationFound = (e) => {
        ...
        if (latitude !== undefined && longitude !== undefined) {
            ...
            if (mapMarker) {
                ...
                newMarker.on('dragend', onMarkerDragEnd);
            }
            ...
            if (accuracyCircle) {
                ...
                const circle = L.circle([latitude, longitude], { radius: accuracy, color: 'blue', fillColor: 'blue', fillOpacity: 0.2 }).addTo(map);
    ...
    const onLocationError = (e) => {
        ...
        setLocation(bhadrakCoordinates[0] + ", " + bhadrakCoordinates[1]);
        setAccuracy(null);
        if (mapMarker) {
            mapMarker.setLatLng(bhadrakCoordinates);
        } else if (map) {
            const newMarker = L.marker(bhadrakCoordinates, { draggable: true }).addTo(map);
            setMapMarker(newMarker);
            newMarker.on('dragend', onMarkerDragEnd);
        }
        if (accuracyCircle) {
            accuracyCircle.setLatLng(bhadrakCoordinates);
            accuracyCircle.setRadius(0);
        } else if (map) {
            const circle = L.circle(bhadrakCoordinates, { radius: 0, color: 'blue', fillColor: 'blue', fillOpacity: 0.2 }).addTo(map);
            setAccuracyCircle(circle);
        }
        map.panTo(bhadrakCoordinates);
    };
        ...
        <div className="report-emergency-container">
            <div className="report-emergency-form">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="form-group">
                        <button type="button" className="recenter-button" onClick={recenterMap} aria-label="Recenter map to marker">
                            Recenter Map
                        </button>
                        <div ref={mapRef} className="map-container"></div>
                        {accuracy !== null && (
                            <p className="accuracy-info">Location accuracy: {Math.round(accuracy)} meters</p>
                        )}
                    </div>
                    <div className="form-group">
